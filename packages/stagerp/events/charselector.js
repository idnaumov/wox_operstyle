  ...
  DB.query(
    ...
    function (err, r) {
      ...
      if (r[0]) {
        ...
        chat.send(player, `Добро пожаловать на !{#2C80EF} OPER STYLE!`);
...
mp.events.add("playerQuit", async (player) => {
  ...
  DB.query('SELECT pedClothes from characters WHERE ID = ?', player.getVariable('id'), function(error, results) {
    const jsonString = JSON.stringify(results);

    // Step 2: Parse the JSON string and access "pedClothes" property
    const parsedResults = JSON.parse(jsonString);
    const pedClothesData = JSON.parse(parsedResults[0].pedClothes);
    
    // player.setProp(0, hat, 0); // Головной убор
    // player.setProp(1, glasses, 0); // Очки
    let masks = player.getClothes(1);
    pedClothesData.mask = masks.drawable;

    let torso = player.getClothes(11);
    pedClothesData.top = torso.drawable;

    let pant = player.getClothes(4);
    pedClothesData.pants = torso.pant;

    let shoe = player.getClothes(4);
    pedClothesData.shoes = torso.shoe;

    console.log(pedClothesData)
  })
});
